...
switch ($action) {
    ...
    case 'start_game':
        ...
        $state['question'] = $qs[0];
        // Initialise un tableau vide pour les réponses de chaque question
        $state['answers'] = array_fill(0, count($qs), (object)[]);
        break;
    ...
    case 'submit_answer':
        ...
        // On vérifie si le joueur n'a pas déjà répondu à CETTE question
        if (!isset($state['answers'][$qIdx]->$nick)) {
            $state['answers'][$qIdx]->$nick = $input['answer_index'];
            
            if ($input['is_correct']) {
                // Score : 1000 points max, diminue avec le temps
                $points = max(500, 1000 - (int)($input['response_time'] * 50));
                $state['scores'][$nick] += $points;
            }
        }
        break;
