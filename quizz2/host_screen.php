...
<body class="bg-indigo-700 min-h-screen text-white font-sans overflow-hidden">
    ...
    <div id="leaderboard-ui" class="h-screen hidden flex flex-col items-center justify-center bg-indigo-900 p-10">
        <h1 class="text-5xl font-black text-yellow-400 mb-10 italic">CLASSEMENT</h1>
        <div id="score-list" class="w-full max-w-xl space-y-4"></div>
        <button onclick="nextStep()" class="mt-12 bg-green-500 text-white px-12 py-4 rounded-full font-black text-2xl hover:scale-110 transition shadow-2xl">
            SUIVANT
        </button>
    </div>
    ...
    <script>
        ...
        function update() {
            fetch(`api_live.php?action=get_state&pin=${pin}`).then(r => r.json()).then(data => {
                // Compteur de réponses en direct
                ...
                if (data.status !== localStatus) {
                    ...
                    if (data.status === 'leaderboard') showLeaderboard(data.scores);
