...
<head>
    ...
    <style>
        @keyframes physicsFall { 0% { transform: translateY(-100vh) rotate(0deg); opacity: 1; } 60% { transform: translateY(80vh) rotate(20deg); } 75% { transform: translateY(70vh) rotate(-10deg); } 90% { transform: translateY(80vh) rotate(5deg); } 100% { transform: translateY(120vh) rotate(0deg); opacity: 0; } }
...
<body class="bg-indigo-700 min-h-screen text-white font-sans overflow-hidden">
    ...
    <div id="question-ui" class="flex flex-col h-screen">
        ...
        <div class="grid grid-cols-2 gap-2 p-2 h-1/4 z-20">
            <div class="bg-red-500 rounded-xl flex items-center p-4 text-xl font-bold italic">▲ <span id="opt1" class="ml-3 font-normal"></span></div>
            <div class="bg-blue-500 rounded-xl flex items-center p-4 text-xl font-bold italic">◆ <span id="opt2" class="ml-3 font-normal"></span></div>
            <div class="bg-yellow-500 rounded-xl flex items-center p-4 text-xl font-bold italic">● <span id="opt3" class="ml-3 font-normal"></span></div>
            <div class="bg-green-500 rounded-xl flex items-center p-4 text-xl font-bold italic">■ <span id="opt4" class="ml-3 font-normal"></span></div>
        </div>
    </div>
    ...
    <script>
        const pin = "<?= $_GET['pin'] ?>";
        let localStatus = "";
        let currentQuestionIndex = -1; // Pour surveiller le changement de question
        ...
        function update() {
            fetch(`api_live.php?action=get_state&pin=${pin}`).then(r => r.json()).then(data => {
                // Détection du changement de question (Même si le statut est déjà 'playing')
                if (data.current_q_index !== currentQuestionIndex && data.status === 'playing') {
                    currentQuestionIndex = data.current_q_index;
                    answeredList = []; 
                ...
                if (data.status === 'playing') {
                    ...
                    nicks.forEach(n => {
                        if(!answeredList.includes(n)) {
                            ...
                            if(p) classroom.innerHTML += `<div class="text-center">${renderAv(p, "w-12 h-12")}<p class="text-[8px] font-bold">${p.nickname}</p></div>`;
                            answeredList.push(n);
                        }
                    });
                }
        ...
        function startFinalPodium(data) {
            ...
            const sorted = Object.entries(data.scores).sort((a,b) => b[1]-a[1]).slice(0,3);
            if(sorted[2]) {
                const p3 = data.players.find(x => x.nickname === sorted[2][0]);
            ...
            if(sorted[1]) {
                const p2 = data.players.find(x => x.nickname === sorted[1][0]);
            ...
            if(sorted[0]) {
                const p1 = data.players.find(x => x.nickname === sorted[0][0]);
