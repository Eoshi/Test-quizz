...
<head>
    ...
    <style>
        ...
        .avatar-view { position: relative; width: 150px; height: 150px; background: #e5e7eb; border-radius: 50%; overflow: hidden; border: 4px solid #fff; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1); margin: 0 auto; }
        .locked { filter: grayscale(1); opacity: 0.4; cursor: not-allowed !important; position: relative; }
        .lock-icon { position: absolute; top: -5px; right: -5px; background: #000; color: #fff; font-size: 10px; border-radius: 50%; width: 18px; height: 18px; display: flex; align-items: center; justify-content: center; border: 2px solid #fff; }
    </style>
</head>
<body class="bg-indigo-900 text-white flex flex-col items-center justify-center min-h-screen p-4 font-sans">
    ...
    <div id="setup-ui" class="bg-white text-gray-800 p-6 rounded-3xl shadow-2xl max-w-sm w-full">
        ...
        <button onclick="joinGame()" class="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black mt-6 shadow-xl active:scale-95 transition uppercase tracking-widest">Rejoindre</button>
    </div>
    ...
    <script>
        ...
        function setAura(id) {
            ...
            if(id === 0) { img.classList.add('hidden'); } else { img.src = `personnage/aura/aura${id}.png`; img.classList.remove('hidden'); }
        ...
        function joinGame() {
            ...
            fetch(`api_live.php?action=join&pin=${pin}`, {
                ...
                document.getElementById('setup-ui').innerHTML = `<div class="text-center py-10"><h1 class='text-4xl font-black text-indigo-600 italic mb-4'>C'EST PARTI !</h1><p class='text-gray-500 font-bold'>Regarde l'écran du Maître...</p><div class='mt-6 text-5xl'>🎮</div></div>`;
            });
        }
